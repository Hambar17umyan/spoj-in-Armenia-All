...
void body()
{
	...
	for (i = 0; i < n; i++)
	{
		...
		for (j = i - 1; j >= 0; j--)
		{
			for (k = j - 1; k >= 0; k--)
			{
				if (arr[i] + arr[j] + arr[k] - max(arr[i], max(arr[j], arr[k])) <= max(arr[i], max(arr[j], arr[k])))
				{
					continue;
				}
				double p = (arr[i] + arr[j] + arr[k])/2;
				if (sqrt(p * (p - arr[i]) * (p - arr[j]) * (p - arr[k])))
				{
					if (sqrt(p * (p - arr[i]) * (p - arr[j]) * (p - arr[k])) > mx)
					{
						mx = sqrt(p * (p - arr[i]) * (p - arr[j]) * (p - arr[k]));
					}
				}
			}
		}
