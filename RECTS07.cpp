...
void body()
{
	...
	for (i = 0; i < n; i++)
	{
		cin >> arr[i].first.x >> arr[i].first.y;
		cin >> arr[i].second.x >> arr[i].second.y;
		k = (arr[i].second.y - arr[i].first.y) * (arr[i].second.x - arr[i].first.x);
		for (j = i - 1; j >= 0; j--)
		{
			if (arr[i].second.y >= arr[j].first.y
				&& arr[i].second.y <= arr[j].second.y
				&& arr[i].first.y <= arr[j].first.y
				&& arr[i].second.x >= arr[j].first.x
				&& arr[i].second.x <= arr[j].second.x
				&& arr[i].first.x <= arr[j].first.x
				)//1
			{
